...
# ================= 先加载数据 =================
df, _, cleaned_exists = load_data()

# ================= 侧边栏：先筛选（指标、预览与分析面板共用 filtered_df） =================
...
st.sidebar.caption("基于当前筛选条件")
st.sidebar.metric("楼盘数", len(filtered_df))
if len(filtered_df) > 0:
    st.sidebar.metric("平均价格", f"{filtered_df['House price'].mean():,.0f} 元/㎡")
else:
...
if len(filtered_df) > 0:
    st.dataframe(filtered_df.head(20), use_container_width=True, height=520)
else:
    st.info("当前筛选条件下没有数据，请调整左侧筛选。")
st.markdown("---")
...
def beautify_plotly(fig, title=None, height=None, showlegend=True, show_xgrid=False, show_ygrid=True):
    ...
    fig.update_xaxes(showgrid=show_xgrid, gridcolor="rgba(148,163,184,0.25)", zeroline=False)
    fig.update_yaxes(showgrid=show_ygrid, gridcolor="rgba(148,163,184,0.25)", zeroline=False)
...
with tab1:
    ...
    if filtered_df.empty:
        ...
        with col1:
            ...
            fig1 = beautify_plotly(fig1, height=460, showlegend=False, show_xgrid=True, show_ygrid=False)
        ...
        with col2:
            ...
            fig2 = beautify_plotly(fig2, height=460, showlegend=False, show_xgrid=True, show_ygrid=False)
        ...
        with col3:
            ...
            fig3 = beautify_plotly(fig3, height=420, showlegend=False, show_xgrid=True, show_ygrid=False)
        ...
        with col4:
            ...
            fig4 = beautify_plotly(fig4, height=420, showlegend=False, show_xgrid=True, show_ygrid=False)
